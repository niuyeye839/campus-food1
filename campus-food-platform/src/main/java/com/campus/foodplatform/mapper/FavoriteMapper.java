...
public interface FavoriteMapper {
    @Insert("INSERT INTO favorite(user_id,target_type,target_id,created_at) VALUES(#{userId},#{targetType},#{targetId},NOW())")
    ...
    @Select("SELECT target_id FROM favorite WHERE user_id=#{userId} AND target_type=#{targetType} ORDER BY created_at DESC LIMIT #{offset},#{size}")
